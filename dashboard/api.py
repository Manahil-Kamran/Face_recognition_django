...
from dashboard.serializers import PersonRegistrationSerializer, FaceCodingSerializer
...
class GetAllFaceCodingApiView(APIView):
    def get(self, request):
        # Assuming `model_classes` is a list of person IDs you're filtering on
        records = FaceCoding.objects.all(
          # Equivalent to `Face_Coding.is_current == 1`
        )
        serialized_records = FaceCodingSerializer(records, many=True)
        return Response(serialized_records.data, status=status.HTTP_200_OK)
